...
the delegations spell their write as a lambda so the rule keeps seeing a call. *(Corrected in block
7: the names land with the helper, not in block 8. The rule, still scoped to the import use cases,
reported `ImportAccess`'s two delegations, a lambda write inside `fenced` with no `inTransaction`
parent, so block 7's gate could not pass without them; the rule's case for the two fences moves
with them.)* A `save*` callable
...
| 7 | `fix/p2-debt-fences` | Generic fence, four delegations, two new extensions, three sites *(Corrected in block 7: the rule's boundary names `fenced` and `fencedOver` too, with the rule's case for them; 4.6 says why)* | `usecases/Fences.kt`, `ExportAccess.kt`, `ImportAccess.kt`, `PinAccess.kt`, `BoardAccess.kt`, `PinTagger.kt`, `PinBoardSetter.kt`, `BoardUpdater.kt`, their tests | A22 to A24 |
| 8 | `fix/p2-debt-fence-rule` | The rule renamed, widened, `save*`, boundary names *(Corrected in block 7: the boundary names landed in block 7)*; four inlinings; two transaction moves; the limit *(Corrected in block 8: 451 lines, of which 368 in production against the 200, 192 of them added. The renamed and widened rule and the six sites it reports land together or the gate is red between them, and half the count is bodies re-indented into a transaction lambda or a constructor call; the block does not split.)* | `detekt-rules/.../RowMergedOutsideTransaction.kt` and test, `PinryRuleSetProvider.kt`, `detekt.yml`, `PinCreator.kt`, `SetPinImage.kt`, `UserCreator.kt`, `UserDataImportRunner.kt`, `UserDataExportRequester.kt`, `UserDataExportBuilder.kt`, `docs/backlog.md` | A25 to A28 |
