...
| 8 | `fix/p2-debt-fence-rule` | The rule renamed, widened, `save*`, boundary names *(Corrected in block 7: the boundary names landed in block 7)*; four inlinings; two transaction moves; the limit | `detekt-rules/.../RowMergedOutsideTransaction.kt` and test, `PinryRuleSetProvider.kt`, `detekt.yml`, `PinCreator.kt`, `SetPinImage.kt`, `UserCreator.kt`, `UserDataImportRunner.kt`, `UserDataExportRequester.kt`, `UserDataExportBuilder.kt`, `docs/backlog.md` | A25 to A28 |
