...
class PinListIntegrationTest : IntegrationTest() {
    ...
    @Test
    fun `getting pins without authentication fails with 401`() {
        given()
            .`when`()
            .get("/api/v1/pins")
            .then()
    ...
    @Test
    fun `getting pins with wrong password fails with 401`() {
        val username = "listuser"
        userCreator.createUserWithPassword(username, defaultPassword)

        given()
            .authenticatedAs(username, "wrongpassword")
            .`when`()
            .get("/api/v1/pins")
            .then()
            .statusCode(401)
    ...
    @Test
    fun `getting pins for user with no pins returns empty list`() {
    ...
    fun `getting pins for user with one pin returns that pin`() {
        ...
        given()
            ...
            .body("pins[0].authorId", equalTo(user.id.toString()))
    }

    @Test
    fun `getting pins for user with multiple pins returns all pins`() {
